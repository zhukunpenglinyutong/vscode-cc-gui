...
export function McpServerDialog({ server, existingIds = [], currentProvider = 'claude', onClose, onSave }: McpServerDialogProps) {
  ...
  // Claude-only config scope. Codex is global-only, so the selector is hidden
  // and the value is ignored by the backend for Codex.
  const [scope, setScope] = useState<'user' | 'local'>(server?.scope ?? 'user');
        ...
        <div className="dialog-body">
          ...
          {!isCodexMode && (
            <div className="mcp-scope-row">
              <label className="mcp-scope-label">{t('mcp.serverDialog.scope.label')}</label>
              <select
                className="mcp-scope-select"
                value={scope}
                onChange={(e) => setScope(e.target.value as 'user' | 'local')}
              >
                <option value="user">{t('mcp.serverDialog.scope.user')}</option>
                <option value="local">{t('mcp.serverDialog.scope.local')}</option>
              </select>
              <span className="mcp-scope-hint">
                {scope === 'user' ? t('mcp.serverDialog.scope.userHint') : t('mcp.serverDialog.scope.localHint')}
              </span>
            </div>
          )}
