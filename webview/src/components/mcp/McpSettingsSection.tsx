...
function McpProviderPanel({ currentProvider }: { currentProvider: McpProvider }) {
  ...
  // Codex mutations report success only after config.toml was written: the
  // backend fires these callbacks from the daemon's [MCP_SERVER_MUTATED]
  // receipt (failures arrive via window.showError instead).
  useEffect(() => {
    if (!isCodexMode) {
      return;
    }
  ...
  const confirmDelete = useCallback(() => {
    if (deletingServer) {
      ...
      // Codex: toast + reload happen in the codexMcpServerDeleted callback.
      if (!isCodexMode) {
  ...
  const handleSaveServer = useCallback((server: McpServer) => {
    if (editingServer) {
      if (editingServer.id !== server.id) {
        if (isCodexMode) {
          // Codex rename goes through a single backend op (oldId) instead of a
          // delete+add pair, so a failure never drops the old entry.
