...
import { BASH_TOOL_NAMES, normalizeToolName } from './toolConstants';
...
function normalizeBashToolInput(input: ToolInputRecord): ToolInput {
  const command =
    coerceTextValue(input.command) ??
    coerceTextValue(input.cmd);

  const description =
    coerceTextValue(input.description) ??
    coerceTextValue(input.justification) ??
    coerceTextValue(input.summary);
...
export function normalizeToolInput(name: string | undefined, input: ToolInput | undefined): ToolInput | undefined {
  ...
  if (BASH_TOOL_NAMES.has(normalizedName)) {
    return normalizeBashToolInput(inputRecord);
  }
