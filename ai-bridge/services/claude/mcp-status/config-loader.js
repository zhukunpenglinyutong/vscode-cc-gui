...
async function parseMcpConfig(cwd = null) {
  ...
  // Merge scopes the way Claude Code itself does: global (`user` scope) servers
  // always apply, and this project's (`local` scope) servers are layered on top,
  // winning on a name clash. This matches the settings panel's merged view — a
  // project having its own servers must NOT hide the global ones.
  ...
  const projectDisabled = (projectConfig && projectConfig.disabledMcpServers) || [];
  disabledServers = new Set([...globalDisabled, ...projectDisabled]);

  log('info', projectConfig
    ? '[MCP Config] Merged global + project MCP configuration'
    : '[MCP Config] Using global MCP configuration');
